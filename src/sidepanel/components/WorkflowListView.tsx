...
function RunDialog({ workflow, onClose, onRunStarted }: { workflow: Workflow; onClose: () => void; onRunStarted?: () => void }) {
  ...
  const run = async () => {
    ...
    chrome.runtime.sendMessage({ action: 'runWorkflow', workflowId: workflow.id, versionId: workflow.activeVersionId, variables: values, ownerTabId: tab?.id, ownerWindowId: tab?.windowId, executionMode: workflow.executionMode }, response => {
