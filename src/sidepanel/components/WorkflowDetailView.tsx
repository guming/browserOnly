...
import type { Workflow, WorkflowStep, WorkflowVersion, WorkflowVariable, WorkflowVariableType } from '../../workflows';
...
export function WorkflowDetailView({ workflow, onBack, onSaved }: Props) {
  ...
  const save = async () => {
    ...
    const nextWorkflow: Workflow = { ...workflow, name: name.trim() || workflow.name, description, variables, activeVersionId: nextVersion.id, status: 'active', updatedAt: Date.now() };
